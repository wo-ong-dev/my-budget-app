...
export class TransactionController {
   ...
   * POST /api/transactions/remove-duplicates-by-date-memo-amount
   */
  static async removeDuplicatesByDateMemoAmount(req: Request, res: Response): Promise<void> {
    try {
      const conn = await pool.getConnection();
      try {
        // 1. 날짜+메모+금액 기준 중복 항목 확인
        const [duplicates] = await conn.query(`
          SELECT 
            date, memo, amount, COUNT(*) as cnt
          FROM transactions
          GROUP BY date, memo, amount
          HAVING COUNT(*) > 1
          ORDER BY cnt DESC
          LIMIT 20
        `);
        ...
        if ((duplicates as any[]).length === 0) {
          res.json({
            ok: true,
            data: {
              message: '날짜+메모+금액 기준 중복 항목이 없습니다.',
              deleted_count: 0,
              before_total: 0,
              after_total: 0
            }
        ...
        // 3. 중복 항목 삭제 (날짜+메모+금액이 일치하는 경우, 가장 작은 id만 유지)
        const [result] = await conn.query(`
          DELETE t1 FROM transactions t1
          INNER JOIN transactions t2
          WHERE t1.id > t2.id
        ...
        res.json({
          ...
          data: {
            message: deletedCount > 0 
              ? `날짜+메모+금액 기준 중복 항목 ${deletedCount}개가 삭제되었습니다.`
              : '날짜+메모+금액 기준 중복 항목이 없습니다.',
            deleted_count: deletedCount,
            before_total: beforeTotal,
            after_total: afterTotal,
            remaining_duplicates: (finalCheck as any[])[0].cnt
